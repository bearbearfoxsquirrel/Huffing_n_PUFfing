from numpy import zeros, std, mean
...
class MyNaturalEvolutionStrategy:
    def __init__(self, problem_dimension, fitness_metric, sample_population_size = 10):
        ...
        self.mean_solution = standard_normal(problem_dimension) #initial guess
    ...
    def get_direction_to_head_towards(self, samples, weighted_rewards):
        ...
        direction_to_head = dot(directions.transpose(), weighted_rewards)
        return direction_to_head

    def get_noise_factor(self, fitness_requirement):
        noise_factor = 2 - (self.mean_solutions_fitness / fitness_requirement)
        return noise_factor
    ...
    def get_weighted_reward(self, sample_reward, mean_sample_reward, standard_deviation_of_rewards):
        return ((((sample_reward - mean_sample_reward) / (standard_deviation_of_rewards - self.noise_factor)))
                  / (self.noise_factor))
